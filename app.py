...
<style>
    ...
    div[data-testid="stMetricValue"] { color: #00308F !important; }
    div[data-baseweb="select"] > div { background-color: #e2e8f0 !important; border: 1px solid #cbd5e1 !important; }
    .grade-title-panel {
        font-size: 15px; font-weight: 700; color: #00308F; border-left: 4px solid #00308F;
        padding-left: 8px; margin-bottom: 12px; margin-top: 5px;
    }
</style>
...
with col_title:
    st.markdown("<div style='font-size: 30px; font-weight: 800; color: #000000; padding-top: 28px; letter-spacing: -1.5px; white-space: nowrap;'>2027학년도 대진대학교 수시 입학상담 솔루션</div>", unsafe_allow_html=True)
...
def load_admission_data():
    ...
    for track_name, keyword in targets.items():
        ...
        if matched_file:
            try:
                if matched_file.endswith(".csv"): temp_df = pd.read_csv(matched_file, encoding="utf-8-sig", header=None)
                else: temp_df = pd.read_excel(matched_file, header=None)
                ...
                if idx_mojib != -1:
                    ...
                    for t, b in zip(row_top, row_bottom):
                        t = t.replace("\n", "").strip()
                        b = b.replace("\n", "").strip()
                        if t == b or b == "" or b == "nan": new_cols.append(t)
                        elif t == "" or t == "nan": new_cols.append(b)
                        else: new_cols.append(f"{t}_{b}")
...
if selected_track == "학생부우수자" and selected_dept != "데이터 로딩 실패":
    ...
    with st.expander(f"🗣️ [{selected_dept}] 면접 기출 문항 확인", expanded=False):
        ...
        else: st.warning("선택하신 학과의 계열 정보가 매핑되지 않았습니다. 대학별 고사 안내서를 확인해주세요.")
...
def get_empty_df(): return pd.DataFrame([{"과목명": None, "이수단위": "", "등급/성취도": ""} for _ in range(15)])

col_config = {
    "과목명": st.column_config.SelectboxColumn("과목명", options=["국어", "영어", "수학", "사회", "한국사", "과학"], required=False)
}
...
def calculate_score(df_list):
    ...
    for df in df_list:
        for _, row in df.iterrows():
            ...
            if unit == 0.0 and grade_str not in ["", "NAN", "NONE"]: unit = 1.0
    ...
    df_car = pd.DataFrame({"unit": car_u, "grade": car_g}).sort_values(by=["grade", "unit"], ascending=[True, False]).head(8)
    df_gen = pd.DataFrame({"unit": gen_u, "grade": gen_g})
    df_pool = pd.concat([df_gen, df_car], ignore_index=True).sort_values(by=["grade", "unit"], ascending=[True, False]).head(18)
...
with col_left:
    with st.container(border=True):
        ...
        if calc_clicked or manual_score > 0:
            if calc_clicked: current_total_consultations = increase_consulting_count()
            ...
            if selected_dept == "데이터 로딩 실패" or selected_dept == "데이터 없음":
                ...
                if str(pred_avg) != "-" and str(pred_cut) != "-" and str(pred_max) != "-":
                    try:
                        ...
                        if final_score <= score_max: bg_color, border_color, icon, status_text, text_color = "#e8f5e9", "#4caf50", "🟢", "안 정", "#2e7d32"
                        elif final_score <= score_avg: bg_color, border_color, icon, status_text, text_color = "#e3f2fd", "#2196f3", "🔵", "적 정", "#1565c0"
                        elif final_score <= score_cut: bg_color, border_color, icon, status_text, text_color = "#fff3e0", "#ff9800", "🟠", "소 신", "#ef6c00"
                        else: bg_color, border_color, icon, status_text, text_color = "#ffebee", "#f44336", "🔴", "상 향", "#c62828"
            ...
            with st.expander(f"🔄 [{selected_track}] 지원 가능 타 학과 추천", expanded=False):
                ...
                if t_name in db and not db[t_name].empty and "모집단위" in db[t_name].columns:
                    ...
                    for d_name in dept_list: 
                        ...
                        rec_avg_27, rec_max_27 = _get_rec(["2027", "환산", "평균"]), _get_rec(["2027", "환산", "최고"])
                        p_avg, p_max = "-", "-"
                        ...
                        if p_avg != "-" and p_max != "-":
                            try:
                                ...
                                if status: rec_data.append({"모집단위 (추천 학과)": d_name, "지원 전략": status, "기준 평균": f"{f_avg:.2f}", "기준 최고": f"{f_max:.2f}"})
                            except: pass
    ...
    if (calc_clicked or manual_score > 0) and selected_dept != "데이터 로딩 실패" and selected_dept != "데이터 없음":
        ...
        with st.container(border=True):
            ...
            def build_safe_row(m_val, a_val, c_val):
                m, a, c = safe_float(m_val), safe_float(a_val), safe_float(c_val)
            ...
            df_grade = pd.DataFrame({
                ...
                "최고_렌더": [m24_r, m25_r, m26_r], "최저_렌더": [c24_r, c25_r, c26_r], "평균": [a24_f, a25_f, a26_f],
            ...
            if not comp_chart_data.isna().all().all():
                ...
                base = alt.Chart(df_comp_long).encode(
                    x=alt.X('연도:N', axis=alt.Axis(labelAngle=0, grid=False, labelFontSize=12, labelFontWeight='bold', domainColor='#cbd5e1', tickColor='#cbd5e1')),
                    y=alt.Y('경쟁률:Q', scale=alt.Scale(zero=False), axis=alt.Axis(title='경쟁률', grid=True, gridDash=[5, 5], gridColor='#e2e8f0', titleAngle=0, titlePadding=20, titleAlign='right', domain=False, ticks=False))
                )
                
                area = base.mark_area(color=alt.Gradient(gradient='linear', stops=[alt.GradientStop(color='#ff4b4b', offset=0), alt.GradientStop(color='white', offset=1)], x1=1, x2=1, y1=1, y2=0), opacity=0.6, interpolate='monotone')
